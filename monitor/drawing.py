...
def draw_toy_alert(annotated_frame, elapsed_time, alert_active):
    """Desenha a interface visual de alerta de brinquedo."""
    if not alert_active:
        return

    text = f"ALERTA: Brinquedo ({int(elapsed_time)}s)"
    font_scale = 1.0
    thickness = 3
    (tw, th), baseline = cv2.getTextSize(
        text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness
